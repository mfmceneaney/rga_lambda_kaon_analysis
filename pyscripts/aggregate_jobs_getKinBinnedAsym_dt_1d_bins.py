...
RGA_LAMBDA_KAON_ANALYSIS_HOME = os.environ['RGA_LAMBDA_KAON_ANALYSIS_HOME']
...
channels   = ['ppimk']
base_dirs  = [
    os.path.abspath(os.path.join(RGA_LAMBDA_KAON_ANALYSIS_HOME,f'jobs/saga/test_getKinBinnedAsym__{rg}__{ch}__1D/')) for rg in run_groups for ch in channels
]
...
ch_sgasym_labels = {
    'dt_rga':{
        ch:{
            'a0':'$\mathcal{A}_{LUT}^{cos(\\Delta\\phi)}$',
            'a2':'$\mathcal{A}_{LUT}^{cos(2\\Delta\\phi)}$',
        } for ch in channels
    }
}
...
xlabel_map = {
    ...
    'zeta_ppim':'$\\zeta_{p\pi^{-}}$', 'xF_pipim':'$x_{F p\pi^{-}}$',
    'z_k':'$z_{K^{+}}$', 'xF_k':'$x_{F K^{+}}$',
    'ptpt':'$P_{\\perp K^{+}} \\cdot P_{\\perp p\\pi^{-}}$ (GeV$^{2}$)',
}

# Arguments for sagas.get_config_list()
result_names = ["a0","a1"] #NOTE: This also gets recycled as the asymmetry name
    ...
    for base_dir, ch_sgasym_label, ch in zip(base_dirs,ch_sgasym_labels,chs):
        ...
        if rescale:
            plot_results_kwargs_base = dict(
                ...
                **{
                    ...
                    'new_sim_path':os.path.abspath(os.path.join(RGA_LAMBDA_KAON_ANALYSIS_HOME,f'jobs/saga/test_getKinBinnedAsym__mc_rga__{ch}__1D/')),
                    'old_sim_path':os.path.abspath(os.path.join(RGA_LAMBDA_KAON_ANALYSIS_HOME,f'jobs/saga/test_getKinBinnedAsym__mc_rga__{ch}__1D/')),
                    'count_key':'count',
                    'yerr_key':'',
        ...
        for binscheme_idx, binscheme_name in enumerate(binschemes.keys()):
            ...
            plot_results_kwargs_base['hist_paths'] = [
                os.path.abspath(os.path.join(RGA_LAMBDA_KAON_ANALYSIS_HOME,f'jobs/saga/test_getBinKinematicsTH1Ds__mc_rga__{ch}/out_binscheme_kinematics.root')),
                os.path.abspath(os.path.join(RGA_LAMBDA_KAON_ANALYSIS_HOME,f'jobs/saga/test_getBinKinematicsTH1Ds__dt_rga__{ch}/out_binscheme_kinematics.root')),
            ]
